...
詳細は [利用可能な予定時刻ファイルの詳細](#利用可能な予定時刻ファイルの詳細) を参照してください。

- Time List データ(拡張子.txt)  
  Multi TIme Viewer 専用フォーマットのイベント定義テキストファイル。  
  １行１イベントでテキストエディタで簡易に作成することができます。

- iCalendar データ(拡張子.ics)  
  一般的な予定表アプリで使われる共通フォーマットのデータ。  
  例えば Google カレンダーのリンクや Outlook で保存したファイルが使えます。
 
- NextTrain フォーマット(拡張子.tbl)  
  次の電車までの時間を表示するためのアプリでよく使われる時刻表データ。  
  時刻表サイトの駅の時刻表から変換して作ることができます。

## 便利な機能

- 再ロード機能  
 一度ロードした予定時刻ファイルは、再度 URL を入力すること無く、再ロードできます。  
   + 今ロードしているタイトルを再ロードしたいときは、一般メニューの ”Reload items” を選んでください。
  + 今のタイトルを含め過去５つまでのタイトルを再ロードしたいときは、リストの先頭にある該当のタイトルのタイトルメニューの ”LOAD" を選んでください。
   + 過去５つ以上前にロードしたタイトルを再ロードしたいときは、スマホの設定画面の History から該当タイトルを選んで SAVE ボタンを押してください。
 
- 長押し機能  
  リスト表示時に ENTER ボタンを長押しすると、あらかじめ設定したメニューの機能を直接実行できます。 
  一般メニューの "Assign Long Push" で設定します。 
   + 一般的には "Goto Next Item" （現在時刻に一番近い時間のアイテムに移動する）を設定しておくのが便利です。
   + 秒表示を頻繁にON/OFFしたいならば "Display Second"  （秒表示On/Off を切り替える）設定してみてください。
   + 並び替えたMoreメニューをすぐに出したいならば "More Menu" を設定するという使い方もできます。

- アラーム機能   
  個々のアイテムには時間が来たら振動するアラームをTime Listデータもしくはメニューで個別に設定できます。  
  アラームが設定されているアイテムには残り時間の前にベルマークのアイコンが表示されます。
  + 個々のアイテムのアラームを個別に On/Off 切り替えできます。
    一般メニューの ”Toggle Alarm” で設定します。
  + アラーム時間が来たら自動的に 本アプリを起動させることが出来ます。  
   一般メニューの "Wakeup by Alarm" で設定します。
  + 現在時刻からちょっと先にアラームを追加したり、任意のイベント前後にアラームを追加したり出来ます。 
    一般メニューの ”Add Now Alarm” や ”Add Alarm item” で設定します。 
  + 全アイテムのアラームを一括On/Offしたり、デフォルトの設定に戻したり出来ます。  
    一般メニューの ”Alarm Mode” で設定します。
  + iCalendar の予定をロードするときに一括してアラームOnにする事も出来ます。
    タイトルメニューの ”iCalendar Alarm” で設定します。

- スマホ切断振動機能
  Watchface によくある、スマホ Pebble App との接続が切れたときに振動する機能です。 
  一般メニューの ”DisConnect Vibe” で設定します。
...
## 通常画面の表示内容と操作方法
  ...
  + リスト表示  
    ...
    * アイテム一覧  
      ...
      アイテム名、設定時間(3行表示時のみ)、残り時間 が表示されます。
  + 操作  
    - Enter  
      タイトルで押したときはタイトルメニュー表示。アイテムで押したときは一般メニュー表示します。
    - Enter 長押し  
      一般メニューの ”Assign Long Push” で設定したメニュー項目を直接実行します。
    - Up  
       リストを上にスクロールします。
    - Down  
       リストを下にスクロールします。
...
並び順はスマホ上の設定画面の ”SORT MENU ITEMS" で自分の好きな順に並び替えができます。

- Item Information  
  アイテム情報を表示する
- Goto Next Item  
  現在時刻に一番近い時間のアイテムに移動する  
  この機能を使うには、アイテムソート順を時間順にしておくこと
- Goto Top Item  
  先頭のアイテムに移動する
  タイトルに移動したい時にも使う
- Push Timeline  
...
表示されるメッセージとその意味は下記の通りです。

- Data Loaded  
  データが正常にロードできました。
- Data Loaded but overflowed  
  データはロードできましたが、アイテム数がいっぱいで読み込めないアイテムがありました。
- Phone is Disconnected  
  スマホのPebble Appとの接続が切れてます。スマホの Pebble App を立ち上げ直してみてください。
- Items are full  
  アイテム数がいっぱいです。もしアラームを追加したい場合は、必要ないアイテムを削除してください。
- All Title Deleted  
  スマホ設定画面からの指示通り、全タイトルを削除しました。
- Local Alarm item Deleted  
  スマホ設定画面からの指示通り、Pebble上で追加したアラームアイテムを削除しました。
- Server Timeout
- File not found
- Server error  
  ファイルサーバーからデータをロードできませんでした。ちょっと待ってからリトライしてみてください。それでも駄目なときはサーバーの状態を確認してください。
...
- iCalendar データ (拡張子.ics)  
  ...
  * 例えば、下記のようなデータが利用できます。
      + Google カレンダーの非公開リンク URL 
          - ロードしたい カレンダーの「カレンダー設定」の中の ICAL(非公開URL)を使ってください。
      + Outlook で保存した iCalendar ファイルを dropbox上に置いたもの
          - 保存時の設定で、今後一週間、タイトルのみ含む、にするのが使いやすいです。
  
  + iCalendar データ利用上の注意
     * iCalender データからロードできるアイテムは、現在時刻の半日前から２週間後までです。

- NextTrain フォーマット(拡張子.tbl)  
  任意の駅の次の電車までの時間を表示するためのアプリでよく使われる時刻表データ。  
  本家のアプリ開発は終了してますが、データフォーマットとしては継続して使われてます。 
  + 本家 [NextTrain on the Web](http://office.toyolab.com/nexttrain/)

  + 時刻表変換サイト [えきから時刻表→NextTrainデータ](http://toshi-u.sakura.ne.jp/tt2tbl/ej2tbl.html) などで任意の駅の時刻表から NextTrain データに変換し、下記の注意事項を元に修正してから dropbox などにアップしてください。
  
  + NextTrain データ修正における注意事項
        * ファイルの拡張子は ".tbl" にしてください。これで NextTrain形式を見分けます
     * ファイルの文字コードは UTF8 で保存してください。シフトJISだと文字化けします。
     * タイトル(#で始まる行)は全角7文字程度に縮めてください。これが画面のリスト先頭のタイトルとして使われます。
     * 行き先や急行種別(アルファベットで始まる行)が１行に入りきらない場合は短くしてください。

   + NextTrain データの利用における注意事項 
     * 読み込むデータは現在時刻から１時間分です。データが古くなったら、リロードしてください。
     * 祝祭日ダイヤを選ぶ場合は Title Menu に追加した"NextTrain Holiday" 設定を"Holiday"にしてください。祝祭日でなければ "Off"にしてください。この変更をしたらリロードしてください。
